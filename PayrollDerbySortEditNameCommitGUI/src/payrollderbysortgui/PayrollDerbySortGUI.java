...
public class PayrollDerbySortGUI extends Application {
    ...
    void modifyTable() {
        ...
        modifyStringFieldListener(tc0, tc);
    }
    void modifyStringFieldListener(TableColumn tc0, TableColumn tc) {
        tc.setOnEditCommit(new EventHandler<CellEditEvent<Employee, String>>() {
            @Override
            public void handle(CellEditEvent<Employee, String> t) {
                Employee employee = ((Employee) t.getTableView().getItems().get(
                        t.getTablePosition().getRow()));
                employee.setEmployeeName(t.getNewValue());
                int rowNumber = t.getTablePosition().getRow();
                int employeeNumberSelected = (int) tc0.getCellObservableValue(rowNumber).getValue();
                PayrollDatabaseOperations.modifyName(t.getNewValue(), employeeNumberSelected);
            }
        });
    }
